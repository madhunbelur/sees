%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% this is the master, I think
% Using Linux Tools
%
% Author: FOSSEE 
% Copyright (c) 2009, FOSSEE, IIT Bombay
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% some changes are being done by Madhu on 4th Jan 2015 (to the version
% he downloaded from git.fossee (some mirror Srikant told a few days ago).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Title page
\title[Using Linux Tools]{Software Development in Engineering and Science (SDES)\\ Using Linux Tools}

\author[FOSSEE] {FOSSEE team\\
(In particular: Puneet Chaganti, C. Madhusudan, Asokan Pichai, 2010)}
...
\begin{frame}[fragile]
  \begin{block}{What is the Linux OS?}
    ...
    \item Free Open Source Operating System
      \begin{description}
        \item[Free] 
          Free as in Free Speech, \alert{not} Free Beer
        \item[Open-Source]
...
\begin{frame}[fragile]
  ...
  \item Hit \texttt{Ctrl + Alt + F1} (learn how to {\em come out} of that {\em first}!)\\
         (Please note: this is keyboard dependent, and GNU/Linux distribution specific.)
  \item Login 
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item Logged in. Where did we reach? 
  ...
  Type \verb|command argument| at the prompt \verb|$  |: i.e.\\
   \verb|$  command argument| \\
  You can change the prompt \verb|$ | (bash syntax: \$PS1).\\
  Some commands do not need an argument.\\
  Almost all commands can be provided with additional options: \verb|$ command -o1 -o2 arguments| \\
...
\begin{frame}[fragile]
  ...
  \end{lstlisting} %$
  \begin{itemize}
  \item   \alert{The GNU/Linux world is case sensitive.}\\
      Commands, arguments, directory names: almost all.\\
      There is a space between command, options, arguments: some options can be combined.\\
      \alert{Avoid spaces in general. In SDES course: spaces (and some more characters) 
            are banned (from filenames)!}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \end{lstlisting}
  \begin{itemize}
  \item Alternately written as \texttt{cd ./sdes} (\texttt{.} : current)
  \item Specifying path relative to \texttt{pwd}
  \item \texttt{..} takes one level \alert{up} the directory structure (\texttt{..} : `parent')
  \end{itemize}
  \begin{lstlisting}
    $ cd ..
  \end{lstlisting} % $
...
\begin{frame}[fragile]
  ...
  \item Hit \texttt{q} to quit the \texttt{man} page. (This is syntax of `less'.)
  \item For more, see the \texttt{man} page of \texttt{man}
  \begin{lstlisting}
    $ man man
  \end{lstlisting} % $
  \item \texttt{less} is more than \texttt{more}.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{lstlisting}
    ...
    /home/user/
    $ ls sdes/
    $ mkdir -p sdes/linux-tools/scripts
  \end{lstlisting} % $
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{lstlisting}
  $ cd /home/user
  # Assume course directory is already created
  $ mv -i sdes/ course/
  \end{lstlisting}
  \begin{itemize}
  \item No prompt! Why?
  ...
  \end{lstlisting} % $
  \begin{itemize}
  \item \texttt{sdes} became a sub-directory of \texttt{course} 
...
\noindent
  ...
  \noindent
    \begin{center}
\hspace*{-3mm}      \begin{tabular}{llp{7cm}}
       {\footnotesize Reference} &  {\small Class } &  {\small Description }                                                      \\
        \hline
...
\begin{frame}[fragile]
  ...
  \item Bash provides tab completion for the following.
    ...
    \item User Names (when they are prefixed with a ``$\sim$'' (tilde) )
...
\begin{frame}[fragile]
  ...
  \item Lists \texttt{file.ext} files, where \texttt{ext} is only one
    character 
  \item See also the \texttt{file} command: no cheating \texttt{file}, though
         tab completions can get cheated
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \end{lstlisting} % $
  \begin{itemize}
  \item \texttt{-v} \alert{inverts} the search
...
\begin{frame}[fragile]
  ...
  \item Could also use \texttt{count = \$\{wc -l wonderland.txt\} }\\
        (instead of the ``open quote'': \texttt{ ` } )
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \end{lstlisting} % $
(indent \alert{recommended} though not obligatory like in Python)
\end{frame}
...
\begin{frame}[fragile]
  ...
  \end{itemize}
  \begin{lstlisting}
    for animal in rat dog man tiger 
    do 
    echo $animal
    done
  \end{lstlisting} % $
